...
const Suppliers: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingSupplier) {
        ...
        await db.suppliers.create(formData, user?.email || 'system');
  ...
  const handleDelete = async (id: number) => {
    ...
    if (!window.confirm('Are you sure? This will hide the supplier from the list.')) return;
    try {
      await db.suppliers.softDelete(id, user?.email || 'system');
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Suppliers</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage raw material vendors and partners.</p>
        </div>
        <button
          onClick={() => {
            setEditingSupplier(null);
        ...
        <AnimatePresence mode="popLayout">
          {isLoading ? (
            ...
            <div className="col-span-full py-20 text-center bento-card">
              ...
              <h3 className="text-lg font-bold text-slate-900 dark:text-white">No suppliers found</h3>
              <p className="text-slate-500 dark:text-slate-400 mt-1">Try adjusting your search or add a new vendor.</p>
            </div>
          ) : (
            filteredSuppliers.map((supplier) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">{supplier.name}</h3>
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-slate-600 dark:text-slate-400">{supplier.phone || 'No phone'}</span>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <span className="text-sm font-medium text-slate-600 dark:text-slate-400 line-clamp-2">{supplier.address || 'No address'}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 dark:text-slate-600">
                      <Hash size={14} />
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs font-bold text-slate-900 dark:text-white data-value">{supplier.gst || 'N/A'}</span>
                    </div>
                  </div>
                </div>

                <div className="mt-8 pt-6 border-t border-slate-50 dark:border-slate-800 flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest">Active Partner</span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                <div>
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{editingSupplier ? 'Edit Supplier' : 'New Supplier'}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium uppercase tracking-wider">Fill in vendor information</p>
                </div>
