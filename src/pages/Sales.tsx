  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Sales</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Manage customer orders and dispatch invoices.</p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-sm">
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    ...
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">No sales recorded</h3>
                    <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">Create your first sales invoice to see it here.</p>
                  </td>
                </tr>
              ) : (
                filteredSales.map((s) => (
                  <tr key={s.id} className="group">
                    <td>
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-slate-900 dark:text-white">{s.invoice_number}</p>
                          <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">ID: {s.id + 2000}</p>
                        </div>
                      </div>
                    </td>
                    <td className="text-slate-600 dark:text-slate-400 font-medium">{format(new Date(s.date), 'dd MMM yyyy')}</td>
                    <td>
                      <div className="flex items-center gap-2">
                        <Users size={14} className="text-slate-400 dark:text-slate-500" />
                        <span className="font-bold text-slate-700 dark:text-slate-300">{s.customer_name}</span>
                      </div>
                    </td>
                    <td>
                      <div className="flex flex-col">
                        <span className="text-emerald-600 dark:text-emerald-400 font-bold">₹{s.total_amount.toLocaleString()}</span>
                        <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider">Incl. ₹{s.transport_cost} Transport</span>
                      </div>
                    </td>
        ...
        <div className="p-6 bg-slate-50/50 dark:bg-slate-800/50 flex items-center justify-between border-t border-slate-100 dark:border-slate-800">
          <p className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Showing {filteredSales.length} results</p>
          <div className="flex items-center gap-2">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                <div>
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Record New Sale</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium uppercase tracking-wider">Enter dispatch and client details</p>
                </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-8 overflow-y-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Users className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500 pointer-events-none" size={18} />
                      <select
                        required
                        className="w-full pl-12 pr-4 py-3 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-slate-900 dark:text-white"
                        value={customerId}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white uppercase tracking-wider">Sales Items</h4>
                    <button
                      type="button"
                      onClick={addItem}
                        ...
                        <div key={index} className={cn(
                          ...
                          <div className="col-span-6 md:col-span-2">
                            ...
                            <input
                              ...
                              type="number"
                              className={cn(
                                "w-full px-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border focus:ring-2 outline-none text-sm transition-all text-slate-900 dark:text-white",
                                isLowStock 
                                  ? "border-rose-300 dark:border-rose-500/50 focus:ring-rose-500/10" 
                                  : "border-slate-200 dark:border-slate-700 focus:ring-indigo-500/10"
                              )}
                              value={item.quantity}
                          ...
                          <div className="col-span-10 md:col-span-2">
                            ...
                            <div className="px-4 py-2.5 text-sm font-bold text-slate-900 dark:text-white bg-white dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800 flex items-center justify-between data-value">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative group">
                      ...
                      <div className="w-full py-8 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl flex flex-col items-center justify-center gap-2 group-hover:border-indigo-400 dark:group-hover:border-indigo-500 group-hover:bg-indigo-50/30 dark:group-hover:bg-indigo-500/5 transition-all">
                        <FileText className="text-slate-400 dark:text-slate-600 group-hover:text-indigo-500 dark:group-hover:text-indigo-400" size={32} />
        ...
        {isViewModalOpen && selectedSale && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="p-8 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between bg-slate-50/50 dark:bg-slate-800/50">
                <div>
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white">Invoice: {selectedSale.invoice_number}</h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-medium uppercase tracking-wider">Dispatch Details</p>
                </div>
              ...
              <div className="p-8 space-y-8">
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-1">
                    ...
                    <p className="text-lg font-bold text-slate-900 dark:text-white">{selectedSale.customer_name}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="label-caps">Dispatch Date</p>
                    <p className="text-lg font-bold text-slate-900 dark:text-white">{format(new Date(selectedSale.date), 'dd MMMM yyyy')}</p>
                  </div>
                </div>
                  ...
                  <table className="w-full text-left border-separate border-spacing-0">
                    ...
                    <tfoot className="bg-slate-50 dark:bg-slate-800/50">
                      <tr>
                        ...
                        <td className="py-3 px-4 text-sm font-bold text-slate-900 dark:text-white text-right data-value">₹{(selectedSale.total_amount - selectedSale.transport_cost).toLocaleString()}</td>
                      </tr>
                      <tr>
                        ...
                        <td className="py-3 px-4 text-sm font-bold text-slate-900 dark:text-white text-right data-value">₹{selectedSale.transport_cost.toLocaleString()}</td>
                      </tr>
                      <tr className="bg-indigo-50 dark:bg-indigo-500/10">
                        ...
                        <td className="py-4 px-4 text-xl font-bold text-indigo-600 dark:text-indigo-400 text-right data-value">₹{selectedSale.total_amount.toLocaleString()}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
